  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 14,
        padding: '14px 16px',
        opacity: isDragging ? 0.5 : debt.status === 'paid' ? 0.65 : 1,
      ...
      {debts.length === 0 ? (
        ...
        <>
          <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
            <SortableContext items={activeDebts.map(d => d.id)} strategy={verticalListSortingStrategy}>
          ...
          {paidDebts.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: activeDebts.length > 0 ? 10 : 0 }}>
              {paidDebts.map(debt => (
                <DebtCard
                  key={debt.id}
                  debt={debt}
